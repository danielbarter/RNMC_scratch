...
if len(sys.argv) != 2:
    print("usage: python refact_PR.py json_file")
    quit()
...
rn = ReactionNetwork.from_input_entries(mol_list)
rn.build()
all_unique_reactions, reactions = ReactionNetwork.identify_concerted_rxns_via_intermediates(rn)
rn = ReactionNetwork.add_concerted_rxns(rn, all_unique_reactions)
